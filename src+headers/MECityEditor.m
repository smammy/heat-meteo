...
- (MECity *)editCity:(MECity *)city otherCities:(NSArray *)others withPrefsWindow:(NSWindow *)prefsWin
{
	int modalVal;
	if (currentCity)
		[currentCity release]; // JRC - was autorelease
    currentCity = [city retain];
    
	if (otherCities)
		[otherCities release]; // JRC - was autorelease
...
//    [resultsTableData deleteRows];
    ...
    [weatherModules addItemsWithTitles:[[MEPlug_inManager defaultManager] moduleNames]];
											...
											row:(int)rowIndex
{
		...
		[aCell isMemberOfClass:[NSPopUpButtonCell class]])
    {
        ...
        else if([key isEqualToString:@"servers"])
        {
            ...
            NSArray *allServers = [[currentCity weatherReport] supportedModuleNames];
...
- (IBAction)performSearch:(id)sender
{
	...
	NSArray *results = [weatherModule performCitySearch:searchWord];
	
	NSAssert(results,@"results was nil in performSearch");
...
- (IBAction)confirmEditing:(id)sender
{
	...
	int row = [cityTable selectedRow];
	NSLog(@"ConfirmEditing: SelectedRow = %d",row);
	...
	NSString *serverName = [weatherModules titleOfSelectedItem];
	NSString *code       = [[[cityTable dataSource] dataForRow:row] objectForKey:@"code"];
	
	[[currentCity weatherReport] addServerNamed:serverName withCode:code];
	
	[NSApp stopModalWithCode:NSOKButton];
// JRC I would like to remove the ability to have an "inactive" server... maybe
/*            else
            {
                ...
                while(mod = [modEnum nextObject])
                    if(![[[currentCity codeAndInfoForServer:mod objectForKey:@"inactive"] boolValue])
                        break;
                ...
                if(!mod)
                {
                    NSBeginAlertSheet(@"No Active Weather Servers",
                                        ...
                                        nil,
                                        nil,
                                        @"Although you have performed at least one search, you have disabled all your weather servers.  Please choose at least one weather server and switch on the check box next to it.");
                }
                else
                    [NSApp stopModalWithCode:NSOKButton];
            }
